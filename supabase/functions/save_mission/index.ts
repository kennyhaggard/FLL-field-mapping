...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!teamName || !pin || !missionName || !mission) {
      return json({ ok: false, error: "Missing teamName/pin/missionName/mission" }, 400);
    }
    ...
    // Upsert based on unique(team_id, name) index
    const { error } = await supabaseAdmin
      .from("missions")
      .upsert(
        { team_id: team.id, name: missionName, mission, updated_at: now },
        { onConflict: "team_id,name" }
      );

    if (error) {
      console.error("Save mission error:", error);
      return json({ ok: false, error: `DB error saving mission: ${error.message}` }, 500);
    }
